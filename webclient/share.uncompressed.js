...
(function() {
  var BCSocket, Connection, Doc, ManagedWebSocket, MicroEvent, ReconnectingWebSocket, SockJS, WebSocket, append, bootstrapTransform, checkValidComponent, checkValidOp, exports, hasBCSocket, hasSockJS, invertComponent, nextTick, socketImpl, strInject, text, transformComponent, transformPosition, types,
    __slice = [].slice,
  ...
  MicroEvent = (function() {
    ...
    MicroEvent.prototype.once = function(event, f) {
      var listener;
      return this.on(event, listener = (function(_this) {
        return function() {
          var args;
  ...
  ManagedWebSocket = (function() {
    ...
    ManagedWebSocket.prototype.debugAll = true;

    function ManagedWebSocket(url) {
      this._disconnect = __bind(this._disconnect, this);
      this._removeMaintainConnectionTimeout = __bind(this._removeMaintainConnectionTimeout, this);
      this._removeEventListeners = __bind(this._removeEventListeners, this);
      this._handleWebsocketError = __bind(this._handleWebsocketError, this);
      this._handleWebsocketMessage = __bind(this._handleWebsocketMessage, this);
      this._handleWebsocketClose = __bind(this._handleWebsocketClose, this);
      this._handleWebsocketOpen = __bind(this._handleWebsocketOpen, this);
      this._handleVisibilityChange = __bind(this._handleVisibilityChange, this);
      this._connect = __bind(this._connect, this);
      this._maintainConnection = __bind(this._maintainConnection, this);
      this._checkHeartbeat = __bind(this._checkHeartbeat, this);
      this._reconnect = __bind(this._reconnect, this);
      this.debug = this.debugAll;
      this.forcedClose = false;
    ...
    ManagedWebSocket.prototype.onopen = function(event) {};

    ManagedWebSocket.prototype.onclose = function(event) {};

    ManagedWebSocket.prototype.onconnecting = function(event) {};

    ManagedWebSocket.prototype.onmessage = function(event) {};

    ManagedWebSocket.prototype.onerror = function(event) {};

    ManagedWebSocket.prototype.send = function(data) {
      if (this.ws) {
        if (this.debug) {
          console.log("ManagedWebSocket", "send", this.url, data);
        }
        return this.ws.send(data);
      } else {
        throw "INVALID_STATE_ERR : Pausing to reconnect websocket";
    ...
    ManagedWebSocket.prototype.close = function() {
      if (this.ws) {
        this.forcedClose = true;
        return this.ws.close();
      }
    ...
    ManagedWebSocket.prototype.refresh = function() {
      if (this.debug) {
        console.log("ManagedWebSocket", "refresh");
      }
      return this._reconnect();
    };

    ManagedWebSocket.prototype._reconnect = function() {
      if (this.debug) {
        console.log("ManagedWebSocket", "reconnect");
      }
      if (!this.ws || this.ws.readyState === WebSocket.CLOSED) {
        return this._connect();
      } else {
    ...
    ManagedWebSocket.prototype._checkHeartbeat = function() {
      if (this.heartbeatResponse === HEARTBEAT_REQUESTED) {
        if (this.debug) {
          console.log("ManagedWebSocket", "no-heartbeat");
        }
        return this._reconnect();
      } else {
        this.heartbeatResponse = HEARTBEAT_REQUESTED;
        if (this.debug) {
          console.log("ManagedWebSocket", "send-heartbeat");
        }
        return this.send(JSON.stringify("heartbeat"));
      }
    };

    ManagedWebSocket.prototype._maintainConnection = function() {
      if (this.debug) {
        console.log("ManagedWebSocket", "healthcheck");
      }
      switch (this.readyState) {
        case WebSocket.CLOSED:
          this._reconnect();
      ...
      return this.maintainConnectionTimeout = setTimeout(this._maintainConnection, HEALTHCHECK_INTERVAL);
    };
    ...
    ManagedWebSocket.prototype._connect = function() {
      ...
      this.readyState = WebSocket.CONNECTING;
      if (this.debug) {
        console.log("ManagedWebSocket", "attempt-connect", this.url);
      }
      this.ws.addEventListener("open", this._handleWebsocketOpen);
      this.ws.addEventListener("close", this._handleWebsocketClose);
      this.ws.addEventListener("message", this._handleWebsocketMessage);
    ...
    ManagedWebSocket.prototype._handleVisibilityChange = function() {
      switch (document.visibilityState) {
        case "visible":
          return this._maintainConnection();
        case "hidden":
          return this._disconnect();
      }
    };

    ManagedWebSocket.prototype._handleWebsocketOpen = function(event) {
      if (this.debug) {
        console.log("ManagedWebSocket", "onopen", this.url);
      }
      this.readyState = WebSocket.OPEN;
      this.heartbeatResponse = HEARTBEAT_NOT_REQUESTED;
      return this.onopen(event);
    ...
    ManagedWebSocket.prototype._handleWebsocketClose = function(event) {
      this.ws = null;
      this.readyState = WebSocket.CLOSED;
      this.onclose(event);
      if (this.forcedClose) {
    ...
    ManagedWebSocket.prototype._handleWebsocketMessage = function(event) {
      var data;
      data = JSON.parse(event.data);
      if (this.debug) {
        console.log("ManagedWebSocket", "onmessage", this.url, event.data);
      }
      if (data.heartbeat) {
        if (this.debug) {
          console.log("ManagedWebSocket", "heartbeat-received", data.heartbeat);
        }
        return this.heartbeatResponse = HEARTBEAT_RECEIVED;
      } else {
        return this.onmessage(event);
    ...
    ManagedWebSocket.prototype._handleWebsocketError = function(event) {
      if (this.debug) {
        console.log("ManagedWebSocket", "onerror", this.url, event);
      }
      return this.onerror(event);
    };

    ManagedWebSocket.prototype._removeEventListeners = function() {
      if (this.debug) {
        console.log("ManagedWebSocket", "remove-event-listeners");
      }
      if (this.ws) {
        this.ws.removeEventListener("open", this._handleWebsocketOpen);
        this.ws.removeEventListener("close", this._handleWebsocketClose);
    ...
    ManagedWebSocket.prototype._removeMaintainConnectionTimeout = function() {
      return clearTimeout(this.maintainConnectionTimeout);
    };

    ManagedWebSocket.prototype._disconnect = function() {
      if (this.debug) {
        console.log("ManagedWebSocket", "disconnect");
      }
      if (this.ws) {
        this._removeEventListeners();
        this._removeMaintainConnectionTimeout();
        this.ws.close();
        this.readyState = WebSocket.CLOSED;
        return this.onclose({
          target: this,
  ...
  Connection = (function() {
    function Connection(host, authentication, overrideSocketImpl) {
      this.docs = {};
      this.state = 'connecting';
      if (host.match(/^wss?:/)) {
        socketImpl = 'websocket';
      }
      if (overrideSocketImpl) {
        socketImpl = overrideSocketImpl;
      }
      ...
      this.socket.onconnecting = (function(_this) {
        return function() {
    ...
    Connection.prototype.makeDoc = function(name, data, callback) {
      ...
      this.docs[name] = doc;
      return doc.open((function(_this) {
        return function(error) {
          if (error) {
